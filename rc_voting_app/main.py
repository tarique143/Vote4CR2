...
def get_settings():
    try:
        res = requests.get(f"{API_URL}/settings"); res.raise_for_status()
        return res.json()
    except requests.exceptions.RequestException:
        return {"election_status": "Closed", "roll_number_rule": "Optional", "show_vote_counts_to_students": False}

def update_settings(settings):
    try:
        res = requests.post(f"{API_URL}/settings", json=settings); res.raise_for_status()
        return True
    except requests.exceptions.RequestException:
        st.error("Failed to update settings. Is the API server running?"); return False
...
def vote_for_candidate(candidate_id, roll_no=None):
    ...
    try:
        ...
        st.error(f"Vote Failed: {error_detail}"); return False
...
def reset_voter_session():
    """Resets the voting session for the next student."""
...
def render_admin_panel():
    ...
    with st.form("settings_form"):
        ...
        with col1:
            election_status = st.selectbox("Election Status", ["Open", "Closed"], index=["Open", "Closed"].index(settings['election_status']))
        with col2:
            roll_number_rule = st.selectbox("Roll Number Rule", ["Mandatory", "Optional", "Disabled"], index=["Mandatory", "Optional", "Disabled"].index(settings['roll_number_rule']))
        with col3:
            show_counts = st.checkbox("Show vote counts to students?", value=settings['show_vote_counts_to_students'])
    ...
    with st.expander("Add or Delete Candidates"):
        ...
        with st.form("add_candidate_form", clear_on_submit=True):
            ...
            if st.form_submit_button("Add Candidate"):
                if name and stream and division:
                    add_candidate(name, stream, division, gender)
                    st.rerun()
                else:
        ...
        if candidate_map:
            ...
            if st.button("Delete Selected Candidate", type="primary"):
                delete_candidate(candidate_map[selected_name])
                st.rerun()
    ...
    if not candidates:
        ...
        st.dataframe(df[['name', 'stream', 'division', 'gender', 'votes']])
        st.subheader("🏆 Winners")
        boys = [c for c in candidates if c['gender'] == 'Boy']
        girls = [c for c in candidates if c['gender'] == 'Girl']
        col1, col2 = st.columns(2)
        with col1:
...
def render_student_view():
    ...
    roll_no = None
    if settings['roll_number_rule'] != "Disabled":
        placeholder = "Enter Your Roll Number" + (" (Required)" if settings['roll_number_rule'] == "Mandatory" else " (Optional)")
    ...
    with col1:
        ...
        for cand in boys:
            with st.container(border=True):
                ...
                label = "Vote" + (f" ({cand['votes']} votes)" if settings['show_vote_counts_to_students'] else "")
